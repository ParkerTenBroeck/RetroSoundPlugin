...
//?: bit 6 and 7 are unused?
    ...
    private void operate() {
        ...
        final double modVibrato = ((inst[0] & (utils.BIT6)) != 0) ? VIBRATO[fmctr] * (1 << octave[ch]) : 0;
        ...
        final int modAM = ((inst[0] & (utils.BIT7)) != 0) ? AM[amctr] : 0;
        final boolean modRectify = ((inst[3] & (utils.BIT3)) != 0);
        ...
        final double carVibrato = ((inst[1] & (utils.BIT6)) != 0) ? VIBRATO[fmctr] * (freq[ch] << octave[ch]) / 512. : 0;
        ...
        final int carAM = ((inst[1] & (utils.BIT7)) != 0) ? AM[amctr] : 0;
        final boolean carRectify = ((inst[3] & (utils.BIT4)) != 0);
...
//    Twiddler t = new Twiddler(0.4);
    private int setenvelope(final int[] instrument, final EnvState[] state,
        ...
        final boolean keyscaleRate = ((instrument[(isCarrier ? 1 : 0)] & (utils.BIT4)) != 0);
        ...
        switch (state[ch]) {
            ...
            case RELEASE:
                ...
                boolean d5 = ((instrument[isCarrier ? 1 : 0] & (utils.BIT5)) != 0);
